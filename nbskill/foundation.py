...
# %% auto #0
__all__ = ['NotebookCell', 'NotebookChapter', 'NotebookDocument', 'output_text', 'numbered_notebook_path', 'remove_demo_path',
           'demo_path', 'demo_path_context', 'path_candidates', 'is_valid_ipynb', 'write_demo_notebook',
           'tool_notebook', 'write_tool_notebook', 'write_demo_file', 'api_return', 'api_error', 'failure_map_path',
           ...
           'file_line_count', 'cap_text', 'NotebookResult', 'notebook_result', 'generated_owner',
           'install_nbdev_pre_commit_hooks', 'bootstrap_nbskill_project', 'parse_literal', 'none_if_string',
           ...
           'xml_attrs', 'find_cell_by_id', 'notebook_hash', 'matches_filter', 'chapter_spans', 'one_chapter',
           'ensure_cell_ids', 'validate_code_cells', 'exported_py_path', 'commit_notebook', 'cell_metadata',
           'notebook_metadata', 'nbskill_cell_metadata', 'file_hash', 'stamp_export_metadata',
...
# %% ../nbs/00_foundation.ipynb #ca95a132
def numbered_notebook_path(nbs_path, name, index=None):
    "Return a flat numbered notebook path for a simple module name."
    root = Path(nbs_path)
    name = Path(name).stem
    if not re.fullmatch(r"[A-Za-z_]\w*", name): raise ValueError("name must be a simple Python module name")
    if index is None:
        numbers = [int(path.name.split("_", 1)[0]) for path in root.glob("[0-9][0-9]_*.ipynb")]
        index = max(numbers, default=-1) + 1
    return root / f"{index:02d}_{name}.ipynb"
...
def tool_notebook():
    ...
    cells = [
        mk_cell("# nbskill tool notebook\n\nA tiny notebook for disposable tool tests.", cell_type="markdown", id="tool-title"),
        mk_cell("answer = 42", id="tool-answer"),
        mk_cell("from fastcore.test import test_eq", id="tool-imports"),
        mk_cell("## Arithmetic\n\n`double_answer` gives readers and symbol tools a public function to find.", cell_type="markdown", id="tool-arithmetic-doc"),
        mk_cell("#| export\ndef double_answer(value):\n    \"Return twice the input value.\"\n    return value * 2", id="tool-double-answer"),
        mk_cell("double_answer(answer)", id="tool-example"),
        mk_cell("test_eq(double_answer(21), 42)", id="tool-test"),
    ]
    nb = new_nb(cells)
    nb.metadata["nbskill"] = {"demo": "tool", "version": 1}
    return nb
...
# %% ../nbs/00_foundation.ipynb #6a7068a9
class NotebookResult(dict):
    "Dictionary result with one compact interpreter display."
    def __repr__(self): return self.get("display", dict.__repr__(self))


def notebook_result(data, max_display_chars=6000):
    "Keep complete structured data while bounding the interpreter display."
    text = str(data.get("text", ""))
    capped = cap_text(text, max_output_chars=max_display_chars)
    display = capped["text"]
    if capped["truncated"]:
        display += f"\n\nDisplay budget: {max_display_chars} chars; omitted {capped['omitted_chars']} chars. Set max_display_chars=None to show all."
    return NotebookResult({**data, "display": display, "display_budget": capped})
...
def _generated_owner_from_header(path):
    try: lines = Path(path).read_text(encoding="utf-8", errors="ignore").splitlines()
