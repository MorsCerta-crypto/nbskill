...
@patch
def output_text(self: NotebookCell):
    """Return all visible outputs for this cell as compact plain text."""
    return cell_output_text(self.cell)


@patch
def semantic_type(self: NotebookCell):
    """Return the reader-facing semantic type for this cell."""
    cell_type = self.cell_type
    if cell_type == 'markdown': return 'markdown'
    if cell_type != 'code': return cell_type or 'unknown'
    if cell_matches_type(self.cell, 'test'): return 'test'
    if cell_matches_type(self.cell, 'example'): return 'example'
    if is_exported_code_cell(self.cell): return 'exported code'
    if re.search(r'^\s*assert\b', self.source, re.MULTILINE): return 'test'
    if re.search(r'^\s*#\|\s*hide\b', self.source, re.MULTILINE): return 'hidden'
    return 'example'


@patch
def context_record(self: NotebookCell):
    """Return the context record shape used by read helpers."""
    return {
        'cell_id': self.id,
        'cell_idx': self.idx,
