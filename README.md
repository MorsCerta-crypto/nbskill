...
`nbskill` is notebook-aware tooling for agents and maintainers working in nbdev projects. It provides focused context, structured edits, safe execution, and review through a Python Pyskill.
...
Use `nbskill` to find the relevant project, notebook, chapter, cell, or symbol; edit the right cells; run focused checks in project context; and review the resulting diff and style.
...
| Verify a change | [`diff_nb`](https://MorsCerta-crypto.github.io/nbskill/review.html#diff_nb), [`exec_nb`](https://MorsCerta-crypto.github.io/nbskill/execute.html#exec_nb), and `doctor` show the source change, executed checks, and diagnostics. |
| Review changes | [`diff_nb`](https://MorsCerta-crypto.github.io/nbskill/review.html#diff_nb) and `doctor` focus on behavior changes and notebook hygiene. |
...
`install_nbskill --target codex` installs the local `jupyter-notebooks` skill. It never rewrites an aai-coding checkout or a README.
