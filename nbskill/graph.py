...
# %% auto #0
__all__ = ['notebook_order_problem_lines', 'symbol_usage_summary', 'symbol_graph', 'private_symbol_report']
...
def _order_problem_line(kind, nb_path, item, call, detail):
    line = f" line={call['line']}" if call.get("line") else ""
    return f"- {kind}: {nb_path} id={getattr(item['cell'], 'id', '')}{line} symbol={call['symbol']!r} {detail}"
        ...
        for item in cells:
            ...
            for call in item["calls"]:
                ...
                if later and not call.get("deferred"):
                    lines.append(_order_problem_line(
