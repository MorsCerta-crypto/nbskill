...
from .write import doc4symbol as _doc4symbol
from .write import example4symbol as _example4symbol
...
def _capture_call(func, **kwargs):
    ...
    if result is not None and not chunks: chunks.append(_as_text(result))
...
def create_mcp():
    ...
    def read_nb_tool(
        ...
        "Read a compact, non-JSON notebook view."
    ...
    def show_doc_tool(path: str, symbol: str, context: int = 2, source: bool = False, show_ids: bool = False) -> str:
        ...
        return _capture_call(_show_doc, path=path, symbol=symbol, context=context, source=source, show_ids=show_ids)
    ...
    def diff_nb_tool(path: str, ref_a: str | None = "HEAD", ref_b: str | None = None, adds: bool = True, changes: bool = True, dels: bool = False) -> str:
        ...
        return _capture_call(_diff_nb, path=path, ref_a=ref_a, ref_b=ref_b, adds=adds, changes=changes, dels=dels)

    @mcp.tool(name="doc4symbol")
    def doc4symbol_tool(path: str, symbol: str, text: str, export: bool = True) -> str:
        "Insert markdown documentation before a symbol cell."
        return _capture_call(_doc4symbol, path=path, symbol=symbol, text=text, export=export)

    @mcp.tool(name="example4symbol")
    def example4symbol_tool(path: str, symbol: str, example: str, cell_type: str = "code", export: bool = True) -> str:
        "Insert an example or test cell after a symbol cell."
        return _capture_call(_example4symbol, path=path, symbol=symbol, example=example, cell_type=cell_type, export=export)
    ...
    def apply_nb_tool(spec_path: str = "dev/nbskill-op.toml") -> str:
        ...
        return _capture_call(_apply_nb, spec_path=spec_path)
    ...
    def chstyle_tool(path: str = ".", skip_folder_re: str | None = None, skip_path: str | None = None, strict: bool = False) -> str:
        ...
        return _capture_call(_chstyle, path=path, skip_folder_re=skip_folder_re, skip_path=skip_path, strict=strict)
    ...
    def py2nb_tool(path: str, nbs_path: str = "nbs", dest: str | None = None, class_lines: int = 100, method_lines: int = 10) -> str:
        ...
        return _capture_call(_py2nb, path=path, nbs_path=nbs_path, dest=dest, class_lines=class_lines, method_lines=method_lines)
