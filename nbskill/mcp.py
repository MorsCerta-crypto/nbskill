...
# %% auto #0
__all__ = ['as_text', 'capture_call', 'capture_notebook_call', 'mcp_log_report', 'format_mcp_log_report', 'mcp_tool_result',
           'nbskill_status', 'doctor_tool', 'edit_notebook_tool', 'create_notebook_tool', 'exec_nb_tool',
           'verify_change_tool', 'main']
    ...
    if project_scope:
        ...
        if recent_failures:
            ...
            warnings.append(_warning(
                ...
                "Run doctor(scopes='style') after resolving it when style diagnostics are relevant.",
                tool=last.get("tool"),
            ))
...
def _context_summary_lines(data):
    ...
    if selection.get("kind") == "context_filter":
        lines = [
            f"resolved={data.get('resolved_kind') or 'filter'}",
            f"matches={len(matches)} shown of {selection.get('total_matches', len(matches))}",
        ]
        for item in matches[:5]:
            label = f"{item.get('path', '')} id={item.get('cell_id', '')}".strip()
            definitions = _definition_items(item.get("source", ""))
            if definitions:
                lines.append(f"- {label}:")
                lines.extend(f"  {line}" for line in definitions)
            else:
                lines.append(f"- {label}: {_short_item(item.get('source', ''), limit=480)}")
        return lines
...
_MCP_DIAGNOSTIC_TOOL_CATALOG = {
    ...
    'doctor': {
        ...
        'description': 'Report actionable notebook errors, warnings, and optional style diagnostics for a path.',
        'when_to_use': "Use the default scopes='error,warning' after an edit; add scopes='style' when style diagnostics are relevant.",
...
_MCP_READ_CONTEXT_TOOL_CATALOG = {
    'context': {
        ...
        'usefulness': 'core',
        'tags': ('read', 'context', 'project', 'notebook', 'cell', 'symbol', 'search', 'filter', 'graph'),
        'description': 'Read one notebook-aware context surface and optionally narrow it to matching cells or source.',
        'when_to_use': "Use first. Choose view='summary' for orientation or view='full' for source; add chapter, cell_type, query, include_re, or exclude_re to narrow the same result.",
        'combine_with': "Use a qualified target or stable cell id with context before edit_notebook; use include_graph=True only for project placement or reuse.",
    },
}
...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "Use context for both known targets and notebook-aware filtering; its summary view finds the useful slice before a full read. "
        ...
        "exec_nb, verify_change, or doctor. doctor reports actionable errors and warnings; use scopes='style' for style diagnostics. "
        "Responses have a short human summary and structured facts, omitting empty metadata. "
...
async def _context_tool(
    ...
    "Read and filter one notebook-aware context surface."
    ...
    max_chars_per_cell = _mcp_clamp_int(max_chars_per_cell, 1200, 0, 20000, "max_chars_per_cell")
    arguments = dict(
        target=target, scope=scope, view=view, chapter=chapter, cell_type=cell_type, query=query,
        include_re=include_re, exclude_re=exclude_re, context_lines=context_lines,
        max_matches=max_matches, max_chars_per_cell=max_chars_per_cell,
        include_output=include_output, line_numbers=line_numbers, include_graph=include_graph,
    ...
    context_args = dict(
        target=target, scope=scope, mode="auto", around=0,
        view="summary" if view == "summary" else "full",
        chapter=chapter, cell_type=cell_type, query=query, include_re=include_re, exclude_re=exclude_re,
        context_lines=context_lines, max_matches=max_matches, max_chars_per_cell=max_chars_per_cell,
        include_output=include_output, line_numbers=line_numbers, verbose=False,
    )
