...
`nbskill` is notebook-aware tooling for agents and maintainers working in nbdev projects. It provides focused context, structured edits, safe execution, and review through Python and MCP.
...
| Use the library directly | Python APIs remain available for integrations and infrequent maintenance operations. |
| Serve agents | `nbskill_mcp` exposes the small MCP workflow. |
...
`nbskill_mcp` is only the stdio transport launcher; notebook work is done through MCP tools or the Python package.

``` python
mcp__nbskill__.healthcheck()
mcp__nbskill__.context(target="nbs/02_write.ipynb#write_nb", mode="overview")
```
