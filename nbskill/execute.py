...
# %% auto #0
__all__ = ['NBSafeShell', 'verify_notebook', 'exec_nb', 'run_notebook_test']
...
from nbskill.foundation import (
    ...
    output_value_text, parse_literal, source_hash, stamp_notebook_metadata, notebook_result,
)
...
# %% ../nbs/03_execute.ipynb #4cb39892
def _execution_evidence(nb, up2id=None):
    executed, skipped, failed = [], [], []
    for _, cell in _executed_cells(nb, up2id=up2id):
        if cell.cell_type != "code": continue
        if _cell_eval_false(cell):
            skipped.append(str(cell.id))
            continue
        executed.append(str(cell.id))
        for output in cell.get("outputs", []):
            if output.get("output_type") == "error":
                failed.append({"cell_id": str(cell.id), "type": output.get("ename", "Error"), "message": output.get("evalue", "")})
    return {"ok": not failed, "executed_cell_ids": executed, "skipped_cell_ids": skipped, "failed_cells": failed, "errors": [f"{item['cell_id']}: {item['type']}: {item['message']}" for item in failed]}

# %% ../nbs/03_execute.ipynb #0d7a8629
def verify_notebook(path, up2id=None, timeout=30, safe=True):
    from nbskill.state import notebook_state
    state = notebook_state(path)
    execution = exec_nb(path, up2id=up2id, timeout=timeout, safe=safe, check_only=True, show_output=False)
    data = dict(execution)
    data.update(kind="verification", text="Verified " + str(path), suggested_cell_ids=[item["cell_id"] for item in state["cells"] if item["state"] in {"dirty", "stale"}], dependency_limits=state["limits"], state_before=state["counts"])
    return notebook_result(data)
