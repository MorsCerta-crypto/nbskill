...
# %% auto #0
__all__ = ['tracked_call', 'project_context', 'context', 'chapter_context', 'file_context', 'symbol_context', 'write_nb',
           'update_cell', 'batch_edit_nb', 'split_nb_chapter', 'exec_nb', 'diff_nb', 'style_check', 'nbskill_validate',
           'py2nb', 'py2nbs', 'new_nbdev_notebook', 'py2nbdev', 'build_nbskill_skill', 'install_nbskill',
           'symbol_graph', 'symbol_connection', 'private_symbol_report', 'store_knowledge', 'add_behaviour_steering',
           'get_knowledge', 'reference_add', 'reference_list', 'reference_ingest', 'reference_query', 'agent_workbench',
...
def project_context(
    ...
    nbskill.read.project_context(path)
...
def chapter_context(
    ...
    nbskill.read.chapter_context(path, query=query, name=name, any_cell_id=any_cell_id)
...
def file_context(
    path: str,  # Notebook path
    include_re: str | None = None,  # Optional regex for markdown and definitions to include
    exclude_re: str | None = None,  # Optional regex for markdown and definitions to exclude
):
    "Show imports, header docs, Markdown, and definition summaries for one notebook."
    nbskill.read.file_context(path, include_re=include_re, exclude_re=exclude_re)
...
def symbol_context(
    path: str,  # Notebook path
    symbol: str,  # Symbol to inspect
    depth: int = 1,  # Callee summary depth; 0 keeps direct implementation context only
):
    "Show implementation, local docs/examples/tests, callers, and callees for a notebook symbol."
    nbskill.read.symbol_context(path, symbol=symbol, depth=depth)
