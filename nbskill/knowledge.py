...
def reference_add(
    ...
    data["repos"][repo_name] = {
        **existing,
        "name": repo_name,
        "url": url,
        "source_kind": "local" if Path(url).expanduser().exists() else existing.get("source_kind", "git"),
        "version": version,
        "package": package,
        "updated_ts": now,
...
def _declared_dependencies(root):
    ...
    values = list((data.get("project") or {}).get("dependencies") or [])
    values.extend(item for group in ((data.get("project") or {}).get("optional-dependencies") or {}).values() for item in group)
    values.extend((data.get("dependency-groups") or {}).values())
    values.extend(((data.get("tool") or {}).get("uv") or {}).get("dev-dependencies") or [])
...
for dist in metadata.distributions():
    ...
    try: direct = json.loads(direct) if direct else None
    except ValueError: direct = None
    rows[name.lower().replace("_", "-")] = {"name": name, "version": dist.version, "direct_url": direct}
print(json.dumps(rows))
"""
    proc = subprocess.run([python, "-c", source], text=True, capture_output=True)
    if proc.returncode: return {}, f"{proc.stderr.strip() or proc.stdout.strip()}"
...
def _workspace_context(current_repo="."):
    ...
    installed, error = _workspace_distributions(python)
    return {
        "root": str(root), "python": python, "python_source": source,
...
def _workspace_package_state(package, workspace):
    ...
    direct = name in workspace["direct_dependencies"]
    if installed and installed.get("direct_url"):
        status = "local_editable" if (installed["direct_url"].get("dir_info") or {}).get("editable") else "direct_url"
    elif installed and direct: status = "direct_installed"
...
def _query_workspace_package(query, package, workspace):
    ...
    for name in workspace["direct_dependencies"]:
...
def reference_propose(
    ...
    if not problem.strip(): raise ValueError("reference proposal needs problem")
    registered = _load_reference_registry(path)["repos"].get(repo)
    if not registered: raise KeyError(f"Unknown reference repo: {repo}")
    workspace = _workspace_context(current_repo)
    state = _workspace_package_state(repo, workspace)
    version = version or state.get("installed_version")
    rows = [row for row in _lance_rows(path, "items") if row.get("repo") == repo]
    if version: rows = [row for row in rows if str(row.get("version")) == str(version)]
    if symbol: rows = [row for row in rows if row.get("symbol") == symbol]
    hit = _public_reference_hit(rows[0]) if rows else None
    title = f"{repo}: {problem.strip().splitlines()[0]}"
    body = "\n".join([
        f"Problem: {problem.strip()}",
        f"Observed from workspace: {Path(current_repo).expanduser().resolve()}",
        f"Reference: {registered.get('url')} at {version or (hit or {}).get('version') or registered.get('resolved_version')}",
...
def reference_query(
    ...
    workspace = _workspace_context(current_repo)
    selected_package = _query_workspace_package(query, package, workspace)
    library = _workspace_package_state(selected_package, workspace) if selected_package else None
    selected_version = version
    source_download = allow_download
    if library and library["installed_version"]:
        selected_version = library["installed_version"]
        if library["direct_url"]: source_download = False
    result = _reference_query_index(
        query, top_k=top_k, include_branch=include_branch, current_repo=current_repo, repos=repos, path=path,
        kind=kind, package=selected_package, version=selected_version, module=module, symbol=symbol,
        include_local=include_local, candidate_k=candidate_k, explain=explain, allow_download=source_download,
    )
    if library:
        library["source_provenance"] = "direct_url" if library["direct_url"] else "pypi"
        for hit in result["hits"]:
            hit["installed_version_match"] = hit.get("version") == library["installed_version"]
            hit["source_provenance"] = library["source_provenance"]
