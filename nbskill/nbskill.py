...
_AAI_ROUTING_NOTICE = "In an nbdev project, load nbskill.skill for an .ipynb or a Python file whose generated_owner(path) returns a notebook. Its module documentation is authoritative. Keep every other file on the normal aai-coding path. Never edit a generated module."
_AAI_SKILL_RULE = f"- Use exhash for ordinary edits. {_AAI_ROUTING_NOTICE}"
_AAI_NBDEV_NOTICE = "This is an nbdev project: notebooks in `nbs/` are source and exported `.py` files are generated. Invoke persistent-python. Its live SKILL.md has the notebook routing rule."
_AAI_NBDEV_LINE = f"NBDEV_MSG = {_AAI_NBDEV_NOTICE!r}"
...
def _remove_marked_block(path):
    start, end = "<!-- nbskill-integration:start -->", "<!-- nbskill-integration:end -->"
    text = path.read_text(encoding="utf-8") if path.exists() else ""
    if start not in text: return False
    if text.count(start) != 1 or text.count(end) != 1:
        raise ValueError(f"Expected one nbskill integration block in {path}")
    before, rest = text.split(start, 1)
    _, after = rest.split(end, 1)
    return _write_if_changed(path, before.rstrip() + after)

# %% ../nbs/06_skill.ipynb #5dca7a88
def _ensure_aai_skill(path):
    text = path.read_text(encoding="utf-8")
    if _AAI_ROUTING_NOTICE in text: return False
...
def _ensure_aai_harness(path):
    ...
    if _AAI_NBDEV_NOTICE not in text:
...
def install_aai_coding_integration(
    ...
    """Install the nbskill routing rule and remove stale integration copies."""
    ...
    for path in [root / "README.md", root / "SETUP.md"]:
        if _remove_marked_block(path): changed.append(str(path.relative_to(root)))
