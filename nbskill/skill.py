"""Notebook-aware Python workflow for nbdev projects.

Use nbskill to inspect, change, execute, and review notebook source without
editing `.ipynb` JSON. Start by reading focused context, make one structured
edit, then run the narrowest useful verification.
...
`context` also accepts a project, chapter, cell id, or public symbol target.
Read it first to obtain stable cell ids and nearby examples before changing a
notebook.
...
Use narrow operations and pass an `expected_hash` when replacing source from
previously read context. `edit_notebook` validates, writes, and exports an
nbdev notebook atomically.
...
Use `exec_nb` for notebook behavior, `diff_nb` for code-cell changes, and
`style_check` for notebook hygiene. The MCP server exposes the same workflow
when structured notebook-tool calls are more convenient than Python.
"""

# This pyskills guide intentionally lives outside a notebook: it documents
# the package's Python surface and is registered directly in pyproject.toml.
...
def _nbskill_mcp_pids():
    try:
        from nbskill.cli import find_nbskill_mcp_processes
        return find_nbskill_mcp_processes(all_projects=True)
    except Exception:
        pass
...
def nbskill_mcp_restart_notice(timeout=5.0, force=True, dry_run=False):
    ...
    try:
        from nbskill.cli import stop_nbskill_mcp_processes
        result = stop_nbskill_mcp_processes(all_projects=True, timeout=timeout, force=force, dry_run=dry_run)
